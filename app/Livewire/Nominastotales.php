...
public function actualizarGrafica()
{
    ...
    foreach ($archivosNominas as $archivoNomina) {
        \Log::info('Procesando archivo', [
            ...
            'archivo' => $archivoNomina->arch_nomina
        ]);

        // Adaptar la expresión regular para el formato "2° Julio" (sin año)
        // O usar una expresión más flexible
        if (preg_match('/^(1°|2°)\s*(\w+)(?:\s+(\d{4}))?$/u', $archivoNomina->periodo, $matches)) {
            ...
            // Calcular el total desde el archivo Excel
            $totalCalculado = $this->calcularTotalDesdeExcel($archivoNomina->arch_nomina);

            \Log::info('Total calculado del Excel', [
...
private function mesTextoANumero(string $mes): int
{
    ...
    /**
     * Calcula el total de pagos desde el archivo Excel de nómina
     * (Reutilizamos la misma lógica del componente anterior)
     */
