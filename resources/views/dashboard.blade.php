                    ...
                    <div x-data="{ menu: 'admin' }" x-on:cambiar-menu.window="menu = $event.detail.menu" class="space-y-4">
                        ...
                        @if (Auth::user()->rol != 'admin')
                            ...
                            <div class="">
                                ...
                                @elseif(Auth::user()->rol == 'AUXILIAR NOMINAS' ||
                                        Auth::user()->solicitudAlta->rol == 'AUXILIAR NOMINAS' ||
                                        Auth::user()->rol == 'Auxiliar Nominas' ||
                                        Auth::user()->solicitudAlta->rol == 'Auxiliar Nominas')
                                    <x-nominas-navbar></x-nominas-navbar>
                                @elseif (Auth::user()->solicitudAlta->departamento == 'Recursos Humanos' ||
                                        Auth::user()->solicitudAlta->rol == 'AUXILIAR RECURSOS HUMANOS' ||
                                        Auth::user()->solicitudAlta->rol == 'AUXILIAR RH' ||
                                        Auth::user()->solicitudAlta->rol == 'AUX RH' ||
                                        Auth::user()->solicitudAlta->rol == 'Auxiliar RH' ||
                                        Auth::user()->solicitudAlta->rol == 'Auxiliar Recursos Humanos' ||
                                        Auth::user()->solicitudAlta->rol == 'Aux RH' ||
                                        Auth::user()->rol == 'AUXILIAR RH' ||
                                        Auth::user()->rol == 'AUXILIAR RECURSOS HUMANOS')
                                    <x-rh-navbar></x-rh-navbar>
                                @elseif (Auth::user()->rol == 'AUXILIAR MONITORISTA' ||
                                        Auth::user()->rol == 'MONITORISTA' ||
                                        Auth::user()->rol == 'Auxiliar Monitorista' ||
                                        Auth::user()->rol == 'Monitorista' ||
                                        Auth::user()->solicitudAlta->departamento == 'Monitoreo')
                                    <x-monitoreo-navbar></x-monitoreo-navbar>
                                @elseif (Auth::user()->rol == 'Auxiliar Administrativo' ||
                                        Auth::user()->rol == 'AUXILIAR ADMINISTRATIVO' ||
                                        Auth::user()->rol == 'Auxiliar administrativo' ||
                                        Auth::user()->solicitudAlta->rol == 'Auxiliar administrativo')
                                ...
                                @elseif(Auth::user()->rol == 'CUSTODIOS' ||
                                        Auth::user()->rol == 'Custodios' ||
                                        Auth::user()->solicitudAlta->rol == 'CUSTODIOS')
                                    <x-custodios-navbar></x-custodios-navbar>
                                @else
