<div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center">
                    ...
                    Historial de Solicitudes de Altas
                </h2>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                    Busque y gestione sus solicitudes de altas
                </p>
            </div>
    ...
    @if($solicitudes->isEmpty())
        <div class="text-center py-12">
            ...
            <p class="text-gray-500 dark:text-gray-400 mb-6">No se encontraron solicitudes que coincidan con su búsqueda.</p>
                        ...
                        @foreach($solicitudes as $solicitud)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">
                                    {{ $loop->iteration + ($solicitudes->currentPage() - 1) * $solicitudes->perPage() }}
                                </td>

                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900 dark:text-white">
                                        {{ trim(($solicitud->nombre ?? '') . ' ' . ($solicitud->apellido_paterno ?? '') . ' ' . ($solicitud->apellido_materno ?? '')) ?: 'No disponible' }}
                                    </div>
                                </td>

                                @if(Auth::user()->rol == 'admin')
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                        {{ $solicitud->solicitante ?? 'No disponible' }}
                                    </td>
                                @else
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                        {{ $solicitud->curp ?? 'No disponible' }}
                                    </td>
                                @endif

                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                    {{ optional($solicitud->created_at)->format('d/m/Y H:i') ?? 'Sin fecha' }}
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <a href="{{ route('sup.solicitud.detalle', $solicitud->id) }}"
