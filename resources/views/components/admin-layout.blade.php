...
foreach ($usuarios as $usuario) {
    ...
    if ($tipo === 'armado') {
        $camposObligatorios = [
            'arch_solicitud_empleo', 'arch_ine', 'arch_nss', 'arch_curp', 'arch_rfc',
            'arch_acta_nacimiento', 'arch_comprobante_estudios', 'arch_comprobante_domicilio',
            'arch_carta_rec_laboral', 'arch_carta_rec_personal', 'arch_cartilla_militar',
            'arch_antidoping', 'arch_carta_no_penales', 'arch_contrato', 'arch_foto',
        ];
    } else {
        $camposObligatorios = [
            'arch_solicitud_empleo', 'arch_ine', 'arch_nss', 'arch_curp', 'arch_rfc',
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-3">
            <div class="flex flex-wrap items-center justify-between">
                <div class="flex flex-wrap items-center gap-4">
                    ...
                    <div class="relative">
                        <button onclick="toggleNotificaciones()" class="flex items-center gap-2 px-4 py-2 text-sm sm:text-base font-medium text-gray-800 dark:text-gray-200 dark:bg-gray-700 hover:text-green-600 dark:hover:text-green-400 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-md transition-all duration-200 relative">
                        ...
                        <div id="notificacionesDropdown" class="hidden absolute right-0 mt-2 w-80 bg-white dark:bg-gray-700 border rounded shadow-lg z-50 max-h-80 overflow-y-auto">
                            @forelse($notificacionesDocumentacion as $alerta)
                                <div class="px-4 py-2 text-sm text-gray-800 dark:text-gray-100 border-b dark:border-gray-600">
                                    <strong>{{ $alerta['nombre'] }}</strong> — <span class="text-xs text-gray-500">{{ $alerta['punto'] }}</span><br>
                                    <span>Documentación incompleta ({{ $alerta['porcentaje'] }}%)</span>
                                </div>
                            @empty
                                <div class="px-4 py-2 text-sm text-gray-600 dark:text-gray-300">No hay usuarios con documentación incompleta.</div>
                            @endforelse
                        </div>
                    </div>
