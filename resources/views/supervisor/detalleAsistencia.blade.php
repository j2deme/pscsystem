            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                ...
                <div class="mb-6 p-4 bg-white dark:bg-gray-800 rounded-lg shadow">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white">
                        Asistencia del {{ \Carbon\Carbon::parse($asistencia->fecha)->format('d/m/Y') }} a las
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                                @foreach ($asistencia->usuarios_enlistados as $usuario)
                                    <div class="bg-gray-100 dark:bg-gray-700 p-3 rounded-lg text-center shadow">
                                        <img src="{{ $usuario->solicitudAlta?->documentacion?->arch_foto ? asset($usuario->solicitudAlta->documentacion->arch_foto) . '?v=' . now()->timestamp : asset('images/default-user.jpg') }}"
                                            class="w-20 h-20 rounded-full mx-auto object-cover mb-2 border border-gray-300 dark:border-gray-600">
                                        <p class="text-gray-800 dark:text-white font-medium">{{ $usuario->name }}</p>
                                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ $usuario->punto }}</p>
                        ...
                        <div>
                            @if ($asistencia->usuarios_descansos->isEmpty())
                                <h4 class="text-md font-semibold text-blue-600 dark:text-blue-400">No se registraron
                                    descansos.</h4>
                            @else
                                <h4 class="text-md font-semibold text-blue-600 dark:text-blue-400 mb-2">Descansaron:
                                </h4>
                                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                                    @foreach ($asistencia->usuarios_descansos as $usuario)
                                        <div class="bg-blue-50 dark:bg-gray-700 p-3 rounded-lg text-center shadow">
                                            <img src="{{ $usuario->solicitudAlta?->documentacion?->arch_foto ? asset($usuario->solicitudAlta->documentacion->arch_foto) . '?v=' . now()->timestamp : asset('images/default-user.jpg') }}"
                                                class="w-20 h-20 rounded-full mx-auto object-cover mb-2 border border-gray-300 dark:border-gray-600">
                                            <p class="text-gray-800 dark:text-white font-medium">{{ $usuario->name }}
                                            </p>
                                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $usuario->punto }}
                                            </p>
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        <div>
                            @if ($asistencia->usuarios_faltantes->isEmpty())
                                <h4 class="text-md font-semibold text-green-600 dark:text-green-400">No se registraron
                                    inasistencias.</h4>
                            @else
                        ...
                        <div>
                            @if ($asistencia->usuarios_coberturas->isEmpty())
                                <h4 class="text-md font-semibold text-yellow-600 dark:text-yellow-400">No se registraron
                                    coberturas.</h4>
                            @else
                                <h4 class="text-md font-semibold text-yellow-600 dark:text-yellow-400 mb-2">Hicieron
                                    cobertura:</h4>
                                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div id="imageModal"
                    ...
                    <img id="modalImage" src="" class="rounded-lg shadow-lg object-contain"
